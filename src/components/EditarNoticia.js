...
const EditarNoticia = () => {
  ...
  useEffect(() => {
    const buscarNoticia = async () => {
      try {
        ...
        // Filtramos la noticia exacta por su ID
  ...
  const handleSubmit = async (e) => {
    ...
    // Solo enviamos archivo si el comandante subió uno nuevo
    if (nuevaImagen) {
    ...
    try {
      ...
      navigate('/panel/noticias'); // Devolvemos al comandante al Arsenal
    } catch (error) {
  ...
  if (cargando) {
    return <div className="text-green-500 animate-pulse font-mono">&gt; Desencriptando archivo seleccionado...</div>;
  }
  ...
  return (
    <div className="bg-black border border-green-500/30 p-8 rounded-lg shadow-[0_0_15px_rgba(0,255,0,0.1)] font-mono max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Módulo de Imagen */}
        <div className="p-4 border border-dashed border-green-900 bg-green-950/5 rounded space-y-4">
          <div className="flex items-center gap-2 text-green-600 text-sm uppercase tracking-widest">
            <ImageIcon className="w-5 h-5" />
            <label>&gt; Reemplazar_Imagen (Opcional)</label>
          </div>
          
          {imagenActual && !nuevaImagen && (
            <div className="text-xs text-green-700 mb-2">
              [ Imagen actual en Cloudinary detectada. Suba una nueva para sobreescribirla. ]
            </div>
          )}

          <input type="file" name="imagen" onChange={handleFileChange} accept="image/*"
            className="w-full text-green-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-bold file:bg-green-900/30 file:text-green-400 hover:file:bg-green-900/50 file:transition-colors file:cursor-pointer" />
        </div>
